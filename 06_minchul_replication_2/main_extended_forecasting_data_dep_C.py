...
# Regularization parameter grid for C value optimization
ngrid = 30
C_values = np.logspace(-3, 1, ngrid)  # This creates 30 values from 0.001 to 10.0
print(f"Regularization grid (C values): {C_values}")
print(f"C values range from {C_values[0]:.6f} to {C_values[-1]:.6f}")

# Cross-validation settings
N_CV_FOLDS = 7  # Number of folds for time series cross-validation
print(f"Using {N_CV_FOLDS}-fold time series cross-validation for C optimization")
...
FORCE_RERUN_SPECIFIC_SETS = []  # This is ignored if FORCE_RERUN_ALL_SETS is True
...
#ALL_POSSIBLE_SETS = ['Full', 'Deter_States_v2']
...
# %% C Value Optimization Function
def optimize_C_value_first_sample(X_train, y_train, C_values, n_splits=7):
    """
...
for PREDICTION_HORIZON in PREDICTION_HORIZONS:
    ...
    # Initialize optimal C values dictionary
    optimal_C_values = {}
    ...
    for i, forecast_date in enumerate(forecast_dates):
        ...
        # --- C Value Optimization (First Training Sample Only) ---
        if i == 0:  # First iteration only
            ...
            for pred_set_name in sets_to_run:
                ...
                if X_train_raw is None or X_train_raw.empty:
                    ...
                    optimal_C_values[pred_set_name] = 1.0
                    continue
                ...
                if len(X_train_aligned) > 50:  # Sufficient data for optimization
                    ...
                    optimal_C = optimize_C_value_first_sample(X_train_aligned, y_train_aligned, C_values, n_splits=N_CV_FOLDS)
                    optimal_C_values[pred_set_name] = optimal_C
                    print(f"    Optimal C for {pred_set_name}: {optimal_C:.6f}")
                else:
                    print(f"    Insufficient data ({len(X_train_aligned)} samples), using default C=1.0")
                    optimal_C_values[pred_set_name] = 1.0
            
            print(f"\n*** C OPTIMIZATION COMPLETE ***")
            print("Summary of optimal C values by predictor set:")
            for pred_set, opt_c in optimal_C_values.items():
                print(f"  {pred_set:20s}: C = {opt_c:.6f}")
            print("These C values will be used for ALL subsequent out-of-sample predictions.\n")
            ...
            for model_name, model_template in MODELS_TO_RUN.items():
                ...
                try:
                    ...
                    if len(X_train_aligned) > max(LAGS_TO_ADD, default=0) + 20:
                        ...
                        # Use optimized C value for Logit_L1 models
                        if 'Logit_L1' in model_name and pred_set_name in optimal_C_values:
                            optimal_C = optimal_C_values[pred_set_name]
                            #model_instance = model_instance.set_params(C=optimal_C , class_weight=None)
                            model_instance = model_instance.set_params(C=optimal_C , class_weight='balanced')
    ...
    results_to_save = {
        ...
        'optimal_C_values': optimal_C_values,
        'C_grid_used': C_values,
        'prediction_horizon': PREDICTION_HORIZON,
        'predictor_sets_analyzed': sets_to_run,
        'methodology': {
            'approach': 'Set-specific C optimization',
            'description': 'Optimal C determined separately for each predictor set using first training sample',
            'cv_method': f'TimeSeriesSplit with {N_CV_FOLDS} folds',
            'optimization_metric': 'Brier Score',
            'total_C_values_tested': len(C_values),
            'C_range': f'{C_values[0]:.6f} to {C_values[-1]:.6f}'
        }
    }
...
print("=" * 80)
print("C Value Selection Approach:")
print("- Grid of 30 C values: np.logspace(-3, 1, 30) = [0.001, ..., 10.0]")
print("- Optimal C determined SEPARATELY for each predictor set")
print("- Each optimal C determined using {}-fold time series cross-validation on FIRST training sample".format(N_CV_FOLDS))
print("- Same optimal C used for ALL subsequent out-of-sample predictions for that predictor set")
print("- This avoids look-ahead bias while still optimizing regularization strength per predictor type")
print("")
print("Number of predictor sets analyzed:", len(optimal_C_values))
print("Optimal C values selected by predictor set:")
for pred_set_name, optimal_C in optimal_C_values.items():
    print(f"  {pred_set_name:20s}: C = {optimal_C:.6f}")
print("")
print("NOTE: Each predictor set gets its own optimal C value because:")
print("- Different predictor sets may have different numbers of features")
print("- Different predictor sets may have different signal-to-noise ratios")
print("- Optimal regularization strength varies by feature dimensionality and data characteristics")
print("")
