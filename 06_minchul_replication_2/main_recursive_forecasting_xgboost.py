        ...
        for pred_set_name in sets_to_run:
            ...
            for model_name, model_template in MODELS_TO_RUN.items():
                ...
                try:
                    ...
                    # Estimation
                    y_train_aligned = y_train_aligned.astype(int).clip(0, 1)
                    X_train_aligned = X_train_aligned.astype(np.float32)
                    X_predict_imputed  = X_predict_imputed.astype(np.float32)
                    ...
                    if 'XGBoost' in model_name:
                        ...
                        model_instance = clone(model_template).set_params(**params)
                    else:
