# Baseline test to verify consistency with main quantile check
...
# Fixed parameters to match main quantile check
H_QT_FIXED = 3
Q_QT_FIXED = 0.25
N_FACTORS_FIXED = 8  # This is what main quantile check uses
...
def run_baseline_test():
    """
    Run baseline test with same parameters as main quantile check
    """
    print(f"\n{'='*60}")
    print(f"Running baseline test: h_qt={H_QT_FIXED}, q_qt={Q_QT_FIXED}, n_factors={N_FACTORS_FIXED}")
    print(f"{'='*60}")
    ...
    for PREDICTION_HORIZON in PREDICTION_HORIZONS:
        ...
        for i, forecast_date in enumerate(forecast_dates):
            ...
            # Generate Predictor Sets with fixed parameters
            predictor_data_iter = {}

            # Generate TFDI with fixed parameters
            TFDI_dis_raw, all_sub_indices = generate_qTrans_Sub_Indices(X_train_imputed, y_train_full, h_qt=H_QT_FIXED, q_qt=Q_QT_FIXED)
            predictor_data_iter['TFDI_pca'] = generate_TFDI_PCA_Factors(TFDI_dis_raw, n_factors=N_FACTORS_FIXED)
            ...
            for model_name, model_template in MODELS_TO_RUN.items():
                ...
                try:
                    ...
                    print(f"Error in model {model_name} at date {forecast_date}: {e}")
...
print(f"  h_qt: {H_QT_FIXED}")
print(f"  q_qt: {Q_QT_FIXED}")
print(f"  n_factors: {N_FACTORS_FIXED}")
...
try:
    ...
    # Save results
    results_filename = f'baseline_test_results_{pd.Timestamp.now().strftime("%Y%m%d_%H%M%S")}.csv'
    ...
    if not results_df.empty:
        for _, row in results_df.iterrows():
            print(f"\nResults:")
            print(f"  h_qt: {row['h_qt']}")
            print(f"  q_qt: {row['q_qt']}")
            print(f"  n_factors: {row['n_factors']}")
            print(f"  Predictor Set: {row['Predictor_Set']}")
            print(f"  Model: {row['Model']}")
            print(f"  Brier Score: {row['Avg_Brier_Score']:.6f}")
            print(f"  Valid Predictions: {row['Valid_Predictions']}/{row['Total_Predictions']}")
    
    print(f"\nBaseline test completed. Results saved to: {results_path}")
